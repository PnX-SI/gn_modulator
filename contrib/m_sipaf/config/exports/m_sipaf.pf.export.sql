WITH pre AS (
     ...
     (
          SELECT
               concat(
                    utilisateurs.bib_organismes.nom_organisme,
                    '/',
                    ref_nomenclatures.t_nomenclatures.label_fr
               ) AS concat_1
          FROM
               utilisateurs.bib_organismes,
               ref_nomenclatures.t_nomenclatures
          WHERE
               utilisateurs.bib_organismes.id_organisme = actors.id_organism
               AND ref_nomenclatures.t_nomenclatures.id_nomenclature = actors.id_nomenclature_type_actor
     ) AS "actors.label_acteur",
     ...
     (
          SELECT
               string_agg(ref_geo.l_areas.area_name, ', ') AS string_agg_2
          FROM
               ref_geo.l_areas,
               pr_sipaf.cor_area_pf,
               ref_geo.bib_areas_types
          WHERE
               pr_sipaf.cor_area_pf.id_passage_faune = pr_sipaf.t_passages_faune.id_passage_faune
               AND pr_sipaf.cor_area_pf.id_area = ref_geo.l_areas.id_area
               AND ref_geo.bib_areas_types.id_type = ref_geo.l_areas.id_type
               AND ref_geo.bib_areas_types.type_code = 'COM'
     ) AS label_communes,
     (
          SELECT
               string_agg(ref_geo.l_areas.area_name, ', ') AS string_agg_3
          FROM
               ref_geo.l_areas,
               pr_sipaf.cor_area_pf,
               ref_geo.bib_areas_types
          WHERE
               ...
               AND pr_sipaf.cor_area_pf.id_area = ref_geo.l_areas.id_area
               AND ref_geo.bib_areas_types.id_type = ref_geo.l_areas.id_type
               AND ref_geo.bib_areas_types.type_code = 'DEP'
     ) AS label_departements,
     (
          SELECT
               string_agg(ref_geo.l_linears.linear_name, ', ') AS string_agg_1
          FROM
               ref_geo.l_linears,
               pr_sipaf.cor_linear_pf
          WHERE
               pr_sipaf.cor_linear_pf.id_passage_faune = pr_sipaf.t_passages_faune.id_passage_faune
               AND pr_sipaf.cor_linear_pf.id_linear = ref_geo.l_linears.id_linear
     ) AS label_infrastructures,
     (
          SELECT
               string_agg(ref_geo.l_areas.area_name, ', ') AS string_agg_4
          FROM
               ref_geo.l_areas,
               pr_sipaf.cor_area_pf,
               ref_geo.bib_areas_types
          WHERE
               ...
               AND pr_sipaf.cor_area_pf.id_area = ref_geo.l_areas.id_area
               AND ref_geo.bib_areas_types.id_type = ref_geo.l_areas.id_type
               AND ref_geo.bib_areas_types.type_code = 'REG'
     ) AS label_regions,
     pr_sipaf.t_passages_faune.largeur_dispo_faune AS largeur_dispo_faune,
     pr_sipaf.t_passages_faune.longueur_traversee AS longueur_traversee,
     ...
     pr_sipaf.t_passages_faune.geom_local,
...
FROM
     pr_sipaf.t_passages_faune
     LEFT OUTER JOIN (
          pr_sipaf.cor_pf_nomenclature_ouvrage_type AS cor_pf_nomenclature_ouvrage_type_1
          JOIN ref_nomenclatures.t_nomenclatures AS nomenclatures_ouvrage_type ON nomenclatures_ouvrage_type.id_nomenclature = cor_pf_nomenclature_ouvrage_type_1.id_nomenclature
     ) ON pr_sipaf.t_passages_faune.id_passage_faune = cor_pf_nomenclature_ouvrage_type_1.id_passage_faune
     LEFT OUTER JOIN (
          pr_sipaf.cor_pf_nomenclature_ouvrage_materiaux AS cor_pf_nomenclature_ouvrage_materiaux_1
          JOIN ref_nomenclatures.t_nomenclatures AS nomenclatures_ouvrage_materiaux ON nomenclatures_ouvrage_materiaux.id_nomenclature = cor_pf_nomenclature_ouvrage_materiaux_1.id_nomenclature
     ) ON pr_sipaf.t_passages_faune.id_passage_faune = cor_pf_nomenclature_ouvrage_materiaux_1.id_passage_faune
     ...
     LEFT OUTER JOIN (
          pr_sipaf.cor_pf_nomenclature_ouvrage_categorie AS cor_pf_nomenclature_ouvrage_categorie_1
          JOIN ref_nomenclatures.t_nomenclatures AS nomenclatures_ouvrage_categorie ON nomenclatures_ouvrage_categorie.id_nomenclature = cor_pf_nomenclature_ouvrage_categorie_1.id_nomenclature
     ) ON pr_sipaf.t_passages_faune.id_passage_faune = cor_pf_nomenclature_ouvrage_categorie_1.id_passage_faune
...
SELECT
     STRING_AGG(DISTINCT "actors.label_acteur", ', ') AS "Acteurs",
     ...
     longueur_traversee AS "Longueur de traversee (de l'animal) (m)",
     ...
     STRING_AGG(
          DISTINCT "nomenclatures_ouvrage_categorie.label_fr",
          ', '
     ) AS "Catégorie d'ouvrage",
     STRING_AGG(
          DISTINCT "nomenclatures_ouvrage_materiaux.label_fr",
          ', '
     ) AS "Matériaux",
     STRING_AGG(
          DISTINCT "nomenclatures_ouvrage_type.label_fr",
          ', '
     ) AS "Type d'ouvrage",
     ...
     source AS "Source",
     STRING_AGG(
          DISTINCT "usages.nomenclature_usage_type.label_fr",
          ', '
     ) AS "Usages",
