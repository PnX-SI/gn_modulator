...
export class BaseTableComponent extends BaseComponent implements OnInit {
  ...
  ajaxRequestFunc = (url, config, paramsTable) => {
    return new Promise((resolve, reject) => {
      ...
      const params = {
        ...paramsTable,
        // tabulator renvoie size dans params
        // => changement de size en page_size dans l'api
      ...
      this._services.mData.getList(this.schemaName(), extendedParams).subscribe(
        (res) => {
          // process lists
  ...
  drawTable(): void {
    this.table = new Tabulator(this.tab, {
      ...
      paginationSize: this.pageSize || this.schemaConfig.utils.page_size,
      pagination: "remote",
      ajaxSorting: true,
  ...
  ngOnChanges(changes: SimpleChanges) {
    for (const [key, change] of Object.entries(changes)) {
      if (["schemaName", "filters"].includes(key)) {
        this.process();
      }
      if (key == "value") {
        // TODO
