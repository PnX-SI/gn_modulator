...
from sqlalchemy import Constraint, sql
from ..errors import (
    SchemaSqlError
)
...
class SchemaSqlConstraint():
    ...
    def slq_txt_unique_key_constraint(self):
        ...
        if self.attr('unique') is None:
            return ''
        txt = (
            '\nALTER TABLE {} ADD CONSTRAINT unique_{}_{} ({})'
            .format(
...
""").format(
    ...
    def sql_txt_nomenclature_type_constraint(self, sql_schema_name, sql_table_name, nomenclature_field_name, nomenclature_type):

        # eviter les noms trop long pour eviter les doublons en cas de coupure
        constraint_name = (
            'check_nom_type_{sql_schema_name}_{sql_table_name}_{nomenclature_field_name}_{nomenclature_type}'
            .format(
...
""").format(
                ...
                nomenclature_field_name=nomenclature_field_name,
