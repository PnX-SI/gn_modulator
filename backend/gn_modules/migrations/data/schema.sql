...
---- sql schema gn_modules

CREATE SCHEMA IF NOT EXISTS gn_modules;
...
---- table gn_modules.t_module_tags

CREATE TABLE gn_modules.t_module_tags (
...
---- gn_modules.t_module_tags primary key constraint

ALTER TABLE gn_modules.t_module_tags
    ADD CONSTRAINT pk_gn_modules_t_module_tags_id_module_tag PRIMARY KEY (id_module_tag);


-- cor gn_modules.cor_module_tag

CREATE TABLE IF NOT EXISTS gn_modules.cor_module_tag (
    id_module_tag INTEGER NOT NULL NOT NULL,
    id_module INTEGER NOT NULL NOT NULL
);


---- gn_modules.cor_module_tag primary keys contraints

ALTER TABLE gn_modules.cor_module_tag
    ADD CONSTRAINT pk_gn_modules_cor_module_tag_id_module_tag_id_module PRIMARY KEY (id_module_tag, id_module);

---- gn_modules.cor_module_tag foreign keys contraints

ALTER TABLE gn_modules.cor_module_tag
    ADD CONSTRAINT fk_gn_modules_cor_module_tag_id_module_tag FOREIGN KEY (id_module_tag)
    REFERENCES gn_modules.t_module_tags (id_module_tag)
    ON UPDATE CASCADE ON DELETE CASCADE;

ALTER TABLE gn_modules.cor_module_tag
    ADD CONSTRAINT fk_gn_modules_cor_module_tag_id_module FOREIGN KEY (id_module)
    REFERENCES gn_commons.t_modules (id_module)
    ON UPDATE CASCADE ON DELETE CASCADE;
