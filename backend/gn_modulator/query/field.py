...
def process_fields(Model, query, fields):
    """
    ...
    for field in fields:
        if not Model.has_property(field):
            continue
        field_to_process, only_field = process_field(Model, field)
    ...
    query = query.options(orm.load_only(*property_fields))
    for field in filter(
        lambda x: not (x in field_to_remove_from_process or x in property_fields),
        fields_to_process,
    ):
        _, query = getModelAttr(Model, query, field, only_fields=only_fields)

    return query


def process_field(Model, field):
    ...
    if Model.is_relationship(field):
        ...
        if field.endswith(".nom_complet") and Model == User:
            only_field.extend([f"{field_to_process}.prenom_role", f"{field_to_process}.nom_role"])
